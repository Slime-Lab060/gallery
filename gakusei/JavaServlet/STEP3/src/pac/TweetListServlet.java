...
public class TweetListServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		Tweet = request.getParameter("tweet");
			...
			if(Name.isBlank() || Tweet.isBlank()) {
				getServletContext().getRequestDispatcher("/tweetinput.html").forward(request, response);
				return;
			}
			//サニタイジング処理
			Name = getESCEncodingString(Name);
			Tweet = getESCEncodingString(Tweet);
